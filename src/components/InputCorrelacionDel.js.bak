...
const InputCorrelacionDel = ({onClose}) => {
 ...
 const handleChange= evt=>{
   ...
   //console.log('InputCorrelacionDel 04 evt:',evt);
    ...
    //console.log('InputCorrelacionDel 06 evt.target.Correlacion.name:',evt.target.Correlacion.name);
     ...
     try {
      ...
      const response = await fetch("https://backendpandoragui.herokuapp.com/dashboard/correlaciones", {
        method: "DELETE",
          headers: {
  ...
  try {
      const response = await fetch("https://backendpandoragui.herokuapp.com/dashboard/grupos", {
        method: "GET",
